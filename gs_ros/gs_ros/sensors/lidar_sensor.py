...
class LidarSensor(BaseSensor):
    """ROS 2 sensor that implements a spherical Lidar, publishing PointCloud2 messages."""
    ...
    def add_sensor(self):
        """Instantiate the Genesis spherical Lidar and setup its PointCloud2 publisher."""
        ...
        def timer_callback(pcd_publisher, add_noise):
            if self.scene.is_built:
                assert lidar._is_built, f"lidar sensor is not built"
        ...
        fov = spherical_pattern.get("fov", (360.0, 30.0))
        n_points = spherical_pattern.get("n_points", (64, 128))
        ...
        # pass the debug kwargs when drawing is actually enabled (otherwise
        # 0.0 is rejected). Let Genesis use its defaults when draw_debug is off.
        lidar_kwargs = dict(
            pattern=pattern_cfg,
        ...
        lidar = self.scene.add_sensor(gs.sensors.Lidar(**lidar_kwargs))

        self.sensor_object = lidar
        ...
        lidar_pub = self.node.create_publisher(
            PointCloud2, f"{self.namespace}/{topic}", lidar_qos_profile
        )
        self.sensor_publishers = [lidar_pub]

        timer = self.node.create_timer(
            1 / frequency, lambda: timer_callback(lidar_pub, add_noise)
        )
        setattr(self, f"{self.sensor_name}_lidar_timer", timer)
